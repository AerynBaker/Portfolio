...
public class CS2ProgrammingWeek5 
{
	...
	static boolean groupSumsTarget6(int start, int[] nums, int target) 
	{
		...
		if((start+1)<nums.length){
			...
			if(nums[start]<target){
				if(groupSumsTarget6(start+1, nums, (target-nums[start]))==true);
					return true;
			}
			if(groupSumsTarget6(start+1, nums, target)==true)
				return true;
		}
	...
	static boolean groupSumsTargetNoAdj(int start, int[] nums, int target) 
	{
		...
		if((start+1)<nums.length){
			if(nums[start]<target){
				if(groupSumsTargetNoAdj(start+2, nums, (target-nums[start]))==true);
					return true;
			}
			if(groupSumsTargetNoAdj(start+1, nums, target)==true)
				return true;
		}
	...
	static boolean groupSumsTarget5(int start, int[] nums, int target) 
	{
		...
		if((start+1)<nums.length){
			...
			if(nums[start]<target){
				if(groupSumsTarget5(start+1, nums, (target-nums[start]))==true);
					return true;
			}
			if(groupSumsTarget5(start+1, nums, target)==true)
				return true;
		}
	...
	static boolean groupSumsTargetClump(int start, int[] nums, int target) 
	{
		...
		if((start+1)<nums.length){
			if(nums[start]<target){
				while(nums[start+depth]==nums[start])
					depth++;
				if(groupSumsTargetClump(start+depth, nums, (target-(nums[start]*depth)))==true);
					return true;
			}
			if(groupSumsTargetClump(start+1, nums, target)==true)
				return true;
		}
	...
	{
		return divideHelper(0,0,0,nums);
	}
	
	static boolean divideHelper(int next, int A, int B, int[] nums)
	{
		if((next+1)<nums.length){
			if(divideHelper(next+1, A+(nums[next]), B, nums) == true)
				return true;
			if(divideHelper(next+1, A, B+(nums[next]), nums) == true)
				return true;
		}
		else if(A==B)
			return true;
	...
	{
		return divideHelperOdd10(0,0,0,nums);
	}
	
	static boolean divideHelperOdd10(int next, int A, int B, int[] nums)
	{
		if((next+1)<nums.length){
			if(divideHelperOdd10(next+1, A+(nums[next]), B, nums) == true)
				return true;
			if(divideHelperOdd10(next+1, A, B+(nums[next]), nums) == true)
				return true;
		}
		else if((A%10==0) && (B%2==1))
			return true;
	...
	{
		return divideHelper53(0,0,0,nums);
	}
	
	static boolean divideHelper53(int next, int A, int B, int[] nums)
	{
		if((next+1)<nums.length){
			if(nums[next]%5==0)
				return divideHelper53(next+1, A+(nums[next]), B, nums);
			else if(nums[next]%3==0)
				return divideHelper53(next+1, A, B+(nums[next]), nums);
			if(divideHelper53(next+1, A+(nums[next]), B, nums) == true)
				return true;
			if(divideHelper53(next+1, A, B+(nums[next]), nums) == true)
				return true;
		}
		else if(A==B)
			return true;
